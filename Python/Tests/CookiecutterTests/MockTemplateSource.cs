...
namespace CookiecutterTests
{
	internal class MockTemplateSource : ILocalTemplateSource
	{
		...
		{
			if (Templates.TryGetValue(Tuple.Create(filter, continuationToken), out Tuple<Template[], global::System.String> result))
			{
		...
		{
			if (!UpdatesAvailable.TryGetValue(repoPath, out global::System.Boolean available))
			{
				available = false;
			}
