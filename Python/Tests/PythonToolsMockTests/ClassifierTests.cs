...
";
		...
		private class ClassifierHelper : IDisposable
		{
			...
			private static IEnumerable<string> ExpandNames(string classifications)
			{
				...
				{
					return ClassificationAbbreviations.TryGetValue(c, out global::System.String name) ? name : null;
			...
			private void CheckClassifierSpans(IEnumerable<ClassificationSpan> spans, string expectedSpans)
			{
				...
				foreach (var span in spans)
				{
					...
					Assert.IsTrue(ClassificationAbbreviations.TryGetValue(match.Groups["code"].Value[0], out global::System.String expected), "Did not understand character '" + match.Groups["code"].Value + "'");
