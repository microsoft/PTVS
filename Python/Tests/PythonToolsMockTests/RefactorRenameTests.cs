...
"
		...
		private class TestPreviewChanges : IVsPreviewChangesService
		{
			...
			{
				pIVsPreviewChangesEngine.GetRootChangesList(out global::System.Object rootList);
			...
			{
				list.GetItemCount(out global::System.UInt32 count);

				Assert.AreEqual(expected.Length, (int)count);
				for (int i = 0; i < expected.Length; i++)
				{
					list.GetText((uint)i, VSTREETEXTOPTIONS.TTO_DEFAULT, out global::System.String text);
					Assert.AreEqual(expected[i].Name, text);

					list.GetExpandable((uint)i, out global::System.Int32 expandable);
					if (expected[i].Children.Length != 0)
					{
						Assert.AreEqual(1, expandable);
						list.GetExpandedList((uint)i, out global::System.Int32 canRecurse, out IVsLiteTreeList subList);
