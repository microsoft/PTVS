...
) | %{ @{path="$build\setup\en-us\$_"; name="$_"} } | ?{ Test-Path "$($_.path)" }

$ErrorActionPreference = "Stop"

$jobs = @()
$jobs += begin_sign_files $AuthenticodeFiles `
         (mkdir "$build\setup\signed" -Force) @("stevdo", "dinov") `
         "Python Tools for Visual Studio" "http://aka.ms/ptvs" "Python Tools for Visual Studio" "python;visual studio" `
         "authenticode"

end_sign_files $jobs

copy "$build\setup\signed\PythonToolsInstaller.msi" "$build\release\PTVS$buildidentifier.msi"
