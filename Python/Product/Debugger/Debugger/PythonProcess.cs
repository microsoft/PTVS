...
namespace Microsoft.PythonTools.Debugger
{
	...
	internal class PythonProcess : IDisposable
	{
		...
		{
			if (!_threads.TryGetValue(e.threadId, out PythonThread thread))
			{
		...
		private void OnLegacyBreakpointFailed(object sender, LDP.BreakpointFailedEvent e)
		{
			...
			if (_breakpoints.TryGetValue(e.breakpointId, out PythonBreakpoint breakpoint))
			{
		...
		private void OnLegacyBreakpointSet(object sender, LDP.BreakpointSetEvent e)
		{
			...
			if (_breakpoints.TryGetValue(e.breakpointId, out PythonBreakpoint unbound))
			{
		...
		{
			if (_threads.TryRemove(e.threadId, out PythonThread thread))
			{
		...
		{
			if (_breakpoints.TryGetValue(e.breakpointId, out PythonBreakpoint unboundBreakpoint))
			{
		...
		private void OnLegacyThreadFrameList(object sender, LDP.ThreadFrameListEvent e)
		{
			...
			_threads.TryGetValue(e.threadId, out PythonThread thread);
		...
		{
			_breakpoints.TryRemove(unboundBreakpoint.Id, out PythonBreakpoint bp);
