...
namespace Microsoft.PythonTools.Debugger
{
	static class Extensions
	{
			...
			while (i < l)
			{
				...
				if (ch == '\\')
				{
					...
					if (ch == 'u' || ch == 'U')
					{
						int len = (ch == 'u') ? 4 : 8;
						int max = 16;
						if (TryParseInt(text, i, len, max, out global::System.Int32 val))
						{
							buf.Append((char)val);
							i += len;
						}
		...
		private static bool TryParseInt(string text, int start, int length, int b, out int value)
		{
			...
			{
				if (HexValue(text[i], out global::System.Int32 onechar) && onechar < b)
				{
		...
		{
			if (!HexValue(ch, out global::System.Int32 value))
			{
