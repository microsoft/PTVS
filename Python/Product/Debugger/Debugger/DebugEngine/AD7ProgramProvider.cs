...
namespace Microsoft.PythonTools.Debugger.DebugEngine
{
	...
	public class AD7ProgramProvider : IDebugProgramProvider2
	{
		...
		int IDebugProgramProvider2.GetProviderProcessData(enum_PROVIDER_FLAGS Flags, IDebugDefaultPort2 port, AD_PROCESS_ID ProcessId, CONST_GUID_ARRAY EngineFilter, PROVIDER_PROCESS_DATA[] processArray)
		{
			...
			{
				if (ErrorHandler.Succeeded(port.GetServer(out IDebugCoreServer3 server)))
				{
					...
					if (dbgServer != null)
					{
						...
						int hr = dbgServer.CreateManagedInstanceInServer(typeof(AD7ProgramProvider).FullName, typeof(AD7ProgramProvider).Assembly.FullName, 0, ref g, out IntPtr remoteProviderPunk);
						try
						{
