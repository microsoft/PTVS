...
namespace Microsoft.PythonTools.Debugger.DebugEngine
{
	...
	public sealed class AD7Engine : IDebugEngine2, IDebugEngineLaunch2, IDebugProgram3, IDebugSymbolSettings100, IThreadIdMapper
	{
		...
		{
			return _threadIdMapping.TryGetValue(vsThreadId, out global::System.Int64 result) ? result : (long?)null;
		}

		private static bool IsDebuggingPythonOnly(IDebugProgram2 program)
		{
			program.GetProcess(out IDebugProcess2 process);

			process.EnumPrograms(out IEnumDebugPrograms2 enumPrograms);
			...
			while (true)
			{
				...
				programs[0].GetEngineInfo(out global::System.String engineName, out Guid engineGuid);
		...
		int IDebugEngineLaunch2.LaunchSuspended(string pszServer, IDebugPort2 port, string exe, string args, string dir, string env, string options, enum_LAUNCH_FLAGS launchFlags, uint hStdInput, uint hStdOutput, uint hStdError, IDebugEventCallback2 ad7Callback, out IDebugProcess2 process)
		{
			...
			{
				if (!Guid.TryParse(exe, out Guid processId))
				{
		...
		private static PythonLanguageVersion GetLanguageVersion(string options)
		{
			...
			if (Enum.TryParse(options, out PythonLanguageVersion langVersion))
			{
				return langVersion;
			}
			if (Version.TryParse(options, out Version ver))
			{
				try
				{
		...
		int IDebugEngineLaunch2.ResumeProcess(IDebugProcess2 process)
		{
			...
			EngineUtils.RequireOk(process.GetPort(out IDebugPort2 port));

			IDebugDefaultPort2 defaultPort = (IDebugDefaultPort2)port;

			EngineUtils.RequireOk(defaultPort.GetPortNotify(out IDebugPortNotify2 portNotify));
		...
		public int EnumCodeContexts(IDebugDocumentPosition2 pDocPos, out IEnumDebugCodeContexts2 ppEnum)
		{
			...
			pDocPos.GetFileName(out global::System.String filename);
		...
		internal void Send(IDebugEvent2 eventObject, string iidEvent, IDebugProgram2 program, IDebugThread2 thread)
		{
			...
			EngineUtils.RequireOk(eventObject.GetAttributes(out global::System.UInt32 attributes));
		...
		private void OnExceptionRaised(object sender, ExceptionRaisedEventArgs e)
		{
			...
			if (_threads.TryGetValue(e.Thread, out AD7Thread thread))
			{
				Send(
			...
			foreach (var engine in AD7Engine._engines)
			{
				...
				if (target != null)
				{
					...
					if (pythonProcess != null)
					{
						...
						{
							if (Uri.TryCreate(process2.TransportQualifier, UriKind.RelativeOrAbsolute, out Uri transportUri) && (pythonProcess.Uri == transportUri))
							{
								return target;
							}
						}
