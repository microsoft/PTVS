...
namespace Microsoft.IronPythonTools.Interpreter
{
	...
	class RemoteInterpreter
	{
		...
		internal ObjectHandle LoadAssemblyByName(string name)
		{
			...
			{
				if (_referencedAssemblies.TryGetValue(name, out Assembly res) ||
		...
		private ObjectIdentityHandle MakeHandle(object member)
		{
			...
			lock (_members)
			{
				...
				if (!_members.TryGetValue(member, out ObjectIdentityHandle handle))
				{
		...
		internal string GetModuleDocumentation(ObjectIdentityHandle handle)
		{
			return CallAndHandle(() =>
			{
				...
				if (!module.Get__dict__().TryGetValue("__doc__", out global::System.Object docValue) || !(docValue is string))
				{
					return String.Empty;
				}
		...
		internal string GetModuleName(ObjectIdentityHandle handle)
		{
			return CallAndHandle(() =>
			{
				...
				if (!module.Get__dict__().TryGetValue("__name__", out global::System.Object name) || !(name is string))
				{
					name = "";
				}
				return (string)name;
		...
		internal ObjectIdentityHandle GetMember(ObjectIdentityHandle from, string name)
		{
			return CallAndHandle(() =>
			{
				...
				if (pyType != null)
				{
					foreach (var baseType in pyType.mro())
					{
						...
						if (curType != null)
						{
							...
							try
							{
								if (dict.TryGetValue(name, out global::System.Object bresult))
								{
				...
				if (TryGetMember(obj, name, true, out global::System.Object result))
				{
