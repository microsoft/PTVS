...
namespace Microsoft.VisualStudioTools.MockVsTests
{
	public sealed class MockVs : IComponentModel, IDisposable, IVisualStudioInstance
	{
		...
		)
		{
			...
			if (CachedInfo.LangServicesByName.TryGetValue(contentType, out LanguageServiceInfo info))
			{
		...
		{
			if (ErrorHandler.Failed(Solution.GetProjectOfUniqueName(items[0], out IVsHierarchy hierarchy)))
			{
