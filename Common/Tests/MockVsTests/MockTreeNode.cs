	...
	class MockTreeNode : ITreeNode
	{
		...
		private void DragOntoThisUIThread(Key modifier, ITreeNode[] source)
		{
			...
			if (target != null)
			{
				...
				ErrorHandler.ThrowOnFailure(dropDataSource.GetDropInfo(out global::System.UInt32 okEffects, out IDataObject data, out IDropSource dropSource));
					...
					if (ErrorHandler.Succeeded(hr))
					{
						ErrorHandler.ThrowOnFailure(
							dropDataSource.OnBeforeDropNotify(
								data,
								effect,
								out global::System.Int32 cancel
							)
						);
