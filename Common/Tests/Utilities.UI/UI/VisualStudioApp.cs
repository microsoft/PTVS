	...
	public class VisualStudioApp : AutomationWrapper, IDisposable
	{
		...
		private static IEnumerable<IVsProject> EnumerateLoadedProjects(IVsSolution solution)
		{
			...
			ErrorHandler.ThrowOnFailure((solution.GetProjectEnum(
				...
				ref guid,
				out IEnumHierarchies hierarchies
			)));
			IVsHierarchy[] hierarchy = new IVsHierarchy[1];
			while (ErrorHandler.Succeeded(hierarchies.Next(1, hierarchy, out global::System.UInt32 fetched)) && fetched == 1)
			{
		...
		)
		{
			...
			if (expectedProjects != null && expectedProjects.Value != projects.Count)
			{
				...
				Assert.AreEqual(
					...
					projects.Count(p =>
					{
						return ErrorHandler.Succeeded(solution.GetUniqueNameOfProject((IVsHierarchy)p, out global::System.String mk)) &&
			...
			var vsProject = string.IsNullOrEmpty(projectName) ?
				...
				{
					if (ErrorHandler.Failed(solution.GetUniqueNameOfProject(p, out global::System.String mk)))
					{
						return false;
					}
			...
			ErrorHandler.ThrowOnFailure(solution.GetGuidOfProject(vsProject, out Guid projGuid));

			ErrorHandler.ThrowOnFailure(vsProject.GetProperty(VSConstants.VSITEMID_ROOT, (int)__VSHPROPID.VSHPROPID_ExtObject, out global::System.Object o));
		...
		public List<IVsTaskItem> WaitForTaskListItems(Type taskListService, int expectedCount, bool exactMatch = true)
		{
			...
			{
				allItems.Clear();
				ErrorHandler.ThrowOnFailure(errorList.EnumTaskItems(out IVsEnumTaskItems items));
			...
			{
				string text;
				if (ErrorHandler.Succeeded(item.Document(out global::System.String document)) &&
